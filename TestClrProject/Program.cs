...
namespace TestClrProject
{
    ...
    {
        public class mockDetectModel : IDetectorModel
        {
            public (int, byte[]) Detect(string Base64Img, int width, int height)
            {
                return (0, null);
            }
        }
        public static unsafe void Main()
        {
            ...
            string PathEyes = "ModelAI\\haarcascade_eye.xml";
            mockDetectModel mockDetect = new mockDetectModel();
            ...
            {
                time.Start();
                verify = faceDetect.Verify(ImgBase64Db, ImgBase64Input);
                time.Stop();
                TimeConsume += time.ElapsedMilliseconds;
            }    

            Console.WriteLine(TimeConsume/100);
            /* try
             {
