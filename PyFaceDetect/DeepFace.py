...
def verify(
    ...
    for img1_content, img1_region, _, image1_np in img1_objs:
        for img2_content, img2_region, _, image2_np in img2_objs:
            reals.append((check_spoofing(image1_np), check_spoofing(image2_np)))
    ...
    real = reals[np.argmin(distances)]
...
def find(
    ...
    if path.exists(db_path + "/" + file_name):
        ...
        for index in pbar:
            ...
            for img_content, _, _ , _ in img_objs:
    ...
    for target_img, target_region, _ , _ in target_objs:
...
def represent(
    ...
    if detector_backend != "skip":
        ...
        img_objs = [(img, img_region, 0, 0)]
    # ---------------------------------

    for img, region, _, img_np in img_objs:
...
def extract_faces(
    ...
    for img, region, confidence, img_np in img_objs:
        resp_obj = {}
        real = check_spoofing(img_np)
