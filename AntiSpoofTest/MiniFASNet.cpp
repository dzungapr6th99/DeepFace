...
namespace AntiSpoofing
{
	...
	{
		conv = register_module("conv", torch::nn::Conv2d(torch::nn::Conv2dOptions(in_c, out_c, kernel).stride(stride).padding(padding).groups(groups)));
		bn = register_module("bn", torch::nn::BatchNorm2d(out_c));
	...
	{
		conv = register_module("conv", torch::nn::Conv2d(torch::nn::Conv2dOptions(in_c, out_c, kernel).stride(stride).padding(padding).groups(groups)));
		bn = register_module("batchnorm2d", torch::nn::BatchNorm2d(out_c));
	}
	Linear_Block::~Linear_Block()
	{
	...
	//	linear = register_module("linear", torch::nn::Linear(512, Embeding_Size, false));
	//	bn = register_module("bn", torch::nn::BatchNorm1d(embedding_size));
	//	drop = register_module("drop", torch::nn::Dropout(drop_p));
	//	prob = register_module("prob", torch::nn::Linear(Embeding_Size, num_classess, false));
	//}
