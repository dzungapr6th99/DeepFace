...
{
	std::cout << "Load pnet file caffe with protoText path " << config.protoText << " and caffemodel path " << config.caffeModel << "\n";
	_net = cv::dnn::readNetFromCaffe(config.protoText, config.caffeModel);
	if (_net.empty())
	{
		std::cout << "Error load pnet file caffe with protoText path " << config.protoText << " and caffemodel path " << config.caffeModel << "\n";
