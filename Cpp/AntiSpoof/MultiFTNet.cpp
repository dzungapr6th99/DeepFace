...
namespace AntiSpoofing
{
	...
	FTGenerator::FTGenerator(int in_channels, int out_channels)
	{
		std::vector<int64_t> kernel3 = { 3,3 };

		ft = torch::nn::Sequential({
			{ "Conv2d", torch::nn::Conv2d(torch::nn::Conv2dOptions(in_channels, 128, kernel3).padding(1)) },
			{ "BatchNorm2d", torch::nn::BatchNorm2d(128) },
			{"ReLU", torch::nn::ReLU(true)},
			{"Conv2d",torch::nn::Conv2d(torch::nn::Conv2dOptions(128, 64, kernel3).padding(1))},
			{"BatchNorm2d",torch::nn::BatchNorm2d(64)},
			{"ReLU",torch::nn::ReLU(true)},

			{ "Conv2d",torch::nn::Conv2d(torch::nn::Conv2dOptions(64, out_channels, kernel3).padding(1))},
			{ "BatchNorm2d",torch::nn::BatchNorm2d(out_channels)},
			{"ReLU",torch::nn::ReLU(true)}
			});
	}
		...
		MultiFTNet::MultiFTNet(int img_chanels, int num_classes, int embedding_size, std::vector<int64_t> conv6_kernel)
		{
			...
			ftGenerator = new FTGenerator(128, 1);
