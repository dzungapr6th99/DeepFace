...
namespace AntiSpoofing
{
	...
	Conv_Block::Conv_Block(int in_c, int out_c, std::vector<int64_t> kernel, std::vector<int64_t> stride, std::vector<int64_t> padding, int groups)
	{
		conv = register_module("conv", torch::nn::Conv2d(torch::nn::Conv2dOptions(in_c, out_c, kernel).stride(stride).padding(padding).groups(groups).bias(false)));
		bn = register_module("bn", torch::nn::BatchNorm2d(torch::nn::BatchNorm2dOptions(out_c)));
		prelu = register_module("prelu", torch::nn::PReLU(torch::nn::PReLUOptions().num_parameters(out_c)));
	}
	...
	{
		conv = register_module("conv", torch::nn::Conv2d(torch::nn::Conv2dOptions(in_c, out_c, kernel).stride(stride).padding(padding).groups(groups).bias(false)));
		bn = register_module("batchnorm2d", torch::nn::BatchNorm2d(torch::nn::BatchNorm2dOptions(out_c)));
	}
	Linear_Block::~Linear_Block()
	{
	...
	Flatten::Flatten()
	{
		flatten = register_module("flattend", torch::nn::Flatten());
	}
	...
	Depth_Wise::Depth_Wise(std::vector<int> c1, std::vector<int> c2, std::vector<int> c3, bool Residual, std::vector<int64_t> kernel, std::vector<int64_t> stride, std::vector<int64_t> padding, int groups)
	{
		...
		conv = new Conv_Block(c1[0], c1[1], kernel_1, stride_1, padding_1, 1);
		conv_dw = new  Conv_Block(c2[0], c2[1], kernel_1, stride, padding, 1);
		project = new Linear_Block(c3[0], c3[1], kernel_1, stride_1, padding_1, 1);
		residual = Residual;
	...
	{
		torch::nn::Module* Modules = new torch::nn::Module[numblock];
		...
		for (int i = 0; i < numblock; i++)
		{
			...
			models->push_back(Depth_Wise(c1_tuple, c2_tuple, c3_tuple, true, kernel, stride, padding, groups));
	...
	SEModule::SEModule(int chanels, int reduction)
	{
		...
		fc1 = register_module("conv", torch::nn::Conv2d(torch::nn::Conv2dOptions(chanels, chanels/reduction, 1).padding(0).bias(false)));
		bn1 = register_module("bn", torch::nn::BatchNorm2d(torch::nn::BatchNorm2dOptions( chanels / reduction)));
		relu = register_module("relu", torch::nn::ReLU(torch::nn::ReLUOptions().inplace(true)));
		fc2 = register_module("conv", torch::nn::Conv2d(torch::nn::Conv2dOptions(chanels / reduction, chanels, 1).padding(0).bias(false)));
		bn2 = register_module("bn", torch::nn::BatchNorm2d(torch::nn::BatchNorm2dOptions(chanels / reduction)));
	...
	Depth_Wise_SE::Depth_Wise_SE(std::vector<int> c1, std::vector<int> c2, std::vector<int> c3, bool _residual, std::vector<int64_t> kernel, std::vector<int64_t> stride, std::vector<int64_t> padding, int groups, int se_reduct)
	{
		std::vector<int64_t> kernel_1({ 1,1 });
		std::vector<int64_t> stride_1({ 1,1 });
		std::vector<int64_t> padding_1({ 0,0 });
		conv = new Conv_Block(c1[0], c1[1], kernel_1, stride_1, padding_1, 1);
		conv_dw = new Conv_Block(c2[0], c2[1], kernel_1, stride, padding, c2[0]);
		project = new Linear_Block(c3[0], c3[1], kernel_1, stride_1, padding_1, 1);
		se_module = new SEModule({ c3[1] }, se_reduct);
		residual = _residual;
	}
	Depth_Wise_SE::~Depth_Wise_SE() {}
	...
	ResidualSE::ResidualSE(std::vector<std::vector<int>> c1, std::vector<std::vector<int>> c2, std::vector<std::vector<int>> c3, int num_block, int groups, std::vector<int64_t> kernel, std::vector<int64_t> stride, std::vector<int64_t> padding, int se_reduct) : Residual::Residual(c1, c2, c3, num_block, groups, kernel, stride, padding)
	{
	...
	MiniFASNet::MiniFASNet(std::vector<int> keep, int embedding_size, std::vector<int64_t> conv6_kernel, float drop_p, int num_classess, int img_chanel)
	{
		...
		conv_1 = new Conv_Block(img_chanel, keep[0], kernel3, stride2, padding1, 1);
		conv_2_dw = new Conv_Block(keep[0], keep[1], kernel3, stride1, padding1, keep[1]);
		...
		conv_23 = new Depth_Wise(c1[0], c2[0], c3[0], false, kernel3, stride2, padding1, keep[3]);
		...
		conv_3 = new Residual(c1, c2, c3, 4, keep[4], kernel3, stride1, padding1);

		c1 = { {keep[16], keep[17]} };
		c2 = { {keep[17], keep[18]} };
		c3 = { {keep[18], keep[19]} };

		conv_34 = new Depth_Wise(c1[0], c2[0], c3[0], false, kernel3, stride2, padding1, keep[19]);
		...
		conv_4 = new Residual(c1, c2, c3, 6, keep[19], kernel3, stride2, padding1);

		c1 = { {keep[37], keep[38]} };
		c2 = { {keep[38], keep[39]} };
		c3 = { {keep[39], keep[40]} };

		conv_45 = new Depth_Wise(c1[0], c2[0], c3[0], false, kernel3, stride2, padding1, keep[40]);

		c1 = { {keep[40], keep[41]}, {keep[43], keep[44]} };
		c2 = { {keep[41], keep[42]}, {keep[44], keep[45]} };
		c3 = { {keep[42], keep[43]}, {keep[45], keep[46]} };

		conv_5 = new Residual(c1, c2, c3, 2, keep[40], kernel3, stride1, padding0);
		conv_6_sep = new Conv_Block(keep[46], keep[47], kernel1, stride1, padding0, 1);
		conv_6_dw = new Linear_Block(keep[47], keep[48], conv6_kernel, stride1, padding0, 1);
		conv_6_flatten = torch::nn::Flatten();
		linear = register_module("linear", torch::nn::Linear(torch::nn::LinearOptions(512, Embeding_Size).bias(false)));
		bn = register_module("bn", torch::nn::BatchNorm1d(torch::nn::BatchNorm1dOptions(embedding_size)));
		drop = register_module("drop", torch::nn::Dropout(torch::nn::DropoutOptions(drop_p)));
		prob = register_module("prob", torch::nn::Linear( torch::nn::LinearOptions(Embeding_Size, num_classess).bias(false)));
	}
	...
	MiniFASNetSE::MiniFASNetSE(std::vector<int> keep, int embedding_size, std::vector<int64_t> conv6_kernel, float drop_p, int num_classess, int img_chanel) : MiniFASNet(keep, embedding_size, conv6_kernel, drop_p, num_classess, img_chanel)
	{
		...
		conv_3 = new ResidualSE(c1, c2, c3, 4, keep[4], kernel3, stride1, padding1, 4);
		...
		conv_4 = new ResidualSE(c1, c2, c3, 6, keep[19], kernel3, stride1, padding1, 4);

		c1 = { {keep[40], keep[41]}, {keep[43], keep[44]} };
		c2 = { {keep[41], keep[42]}, {keep[44], keep[45]} };
		c3 = { {keep[42], keep[43]}, {keep[45], keep[46]} };
		conv_5 = new ResidualSE(c1, c2, c3, 2, keep[40], kernel3, stride1, padding1, 4);
	}
